...
router.get("/pasing/:cur", function (req, res) {
    ...
    var queryString = 'select count(*) as cnt from login'
    getConnection().query(queryString, function (error2, data) {
        ...
        fs.readFile('list.html', 'utf-8', function (error, data) {
            ...
            var queryString = 'select * from login order by id desc limit ?,?';
...
router.get("/delete/:id", function (req, res) {
    ...
    getConnection().query('delete from login where id = ?', [req.params.id], function () {
        res.redirect('/main')
    });
...
router.post("/insert", function (req, res) {
    ...
    var body = req.body;
    getConnection().query('insert into login(id,pwd) values (?,?)', [body.id, body.pwd], function () {
        //응답
        res.redirect('/main');
    })
...
router.get("/edit/:id", function (req, res) {
    ...
    fs.readFile('edit.html', 'utf-8', function (error, data) {
        getConnection().query('select * from login where id = ?', [req.params.id], function (error, result) {
            res.send(ejs.render(data, {
                data: result[0]
            }))
        })
...
router.post("/edit/:id", function (req, res) {
    ...
    var body = req.body;
    getConnection().query(`update login set pwd = ${body.pwd} where id = '${req.params.id}'`, function (error) {
        console.log(error)    
        res.redirect('/main')
    })
})
